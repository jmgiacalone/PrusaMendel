...
motor_mount_translation=[50.5,36,0];
motor_mount_thickness=6;

m8_clearance_hole=6.6;
hole_for_608=19.6;
608_diameter=19;
...
layer_thickness=0.3;
filament_feed_hole_d=2.3;
filament_diameter=1.75;
filament_feed_hole_offset=m6_diameter/2+filament_diameter/2-0.5;//filament_diameter+3.5;
...
idler_short_side=wade_block_depth-0;
idler_hinge_r=m3_diameter/2+3.5;
idler_hinge_rr=m3_diameter/2+5.5;
idler_hinge_width=9.5;
		...
		union()
		{
			...
			translate(idler_fulcrum)
			{
				...
				cube([idler_hinge_r*2,idler_hinge_r*2,idler_short_side-2*idler_hinge_width-0.5],
					center=true);
				translate([0,-idler_hinge_r,0])
				cube([idler_hinge_r*2,idler_hinge_r*2-1,idler_short_side-2*idler_hinge_width-0.5],
					center=true);
				rotate(-30)
				{
...
module block_holes()
{
	...
	difference()
	{
		cylinder(r=idler_hinge_rr+0.5,h=idler_short_side+0.5,center=true,$fn=60);
		cylinder(r=idler_hinge_rr+1,h=idler_short_side-2*idler_hinge_width-0.5,center=true);
	}
...
%			rotate([0,180,0])
			...
			for (mount=[0:1])
			{
				...
				translate([-filament_feed_hole_offset+25*((mount<1)?1:-1),
					...
					wade_block_depth/2])
				rotate([-90,0,0])
				cylinder(r=m4_nut_diameter/2,h=base_thickness+block_bevel_r,$fn=6);	
			}
...
module motor_mount_holes()
{
...

	{
		...
		for (hole=[0:2])
		{
			translate([motor_hole(hole)[0]-slot_left,motor_hole(hole)[1],-0.5])
			cylinder(h=motor_mount_thickness-screw_head_recess_depth+1,r=radius,$fn=16);
			translate([motor_hole(hole)[0]+slot_right,motor_hole(hole)[1],-0.5])
			cylinder(h=motor_mount_thickness-screw_head_recess_depth+1,r=radius,$fn=16);

			translate([motor_hole(hole)[0]-slot_left,motor_hole(hole)[1]-radius,-0.5])
...
module wadeidler() 
{
	difference()
	{
		union()
		{
			...
			rotate([0,0,-30])
			{
				cylinder(h=idler_short_side,r=idler_hinge_rr,center=true,$fn=60);
				translate([-idler_end_length/2,0,0])
				cube([idler_end_length,idler_hinge_rr*2,idler_short_side],center=true);
			}		
		}
		...
		translate(idler_axis)
		{
			...
			cylinder(h=idler_short_side-6,r=m8_diameter/2-tightness/*Tight*/,
				center=true,$fn=20);
		}
		...
		translate(idler_fulcrum+[0,0,-idler_short_side/2])
		rotate(360/16)
		cylinder(h=3,r=m3_nut_diameter/2);		
...
module bowden_mount()
{
	extruder_recess_d=m3_diameter; 
	extruder_recess_h=10;

	// Recess in base
	translate([0,0,-1])
	cylinder(r=extruder_recess_d/2,h=extruder_recess_h);
	//cone
	translate([0,0,22])
	cylinder(r1=filament_feed_hole_d/2,r2=extruder_recess_d*1.4,h=extruder_recess_h-1);

	//nut trap
	translate([0,-10,9]) cube([m3_nut_diameter,20,8.8], center=true);
	translate([0,0,9]) rotate([0,0,90]) cylinder(r=m3_nut_diameter/sqrt(3),h=8.8, $fn=6, center=true);

	//access slot
	translate([0,-10,0]) cube([extruder_recess_d,20,20], center=true);
}