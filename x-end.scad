...
axis_diameter_larger = 4.15;//4.5 ;
pressure_pad_height = 0.5;

module xend(zheight=45){
difference(){
union(){
translate(v = [0, 9.5, 0])bushing(true,60);
...
//chop the top off
//translate([0,-30,zheight+50-24.5]) cube([25,25,100], center=true);
...
union(){
	...
	translate(v = [0, 0, 2.5]) cylinder(h = 90, r=m8_diameter/2, $fn=9, center=true);
}
}
...
translate([-12.5,-5,zheight]) cube([25,25,100]);
}
}
...
module xendtest()
{
	difference(){
		xend();
		xendcorners(5,5,5,5,0);
		translate([0,0,55.8]) cube([100,100,80], center=true);
	}
}
