...
translate([-15,-60,0]) rotate([0,0,30])
	vertex(true);
translate([25,-60,0]) rotate([0,0,30])
	vertex(true);

module zhole(diameter) #cylinder(h=18,r=(diameter/(2*cos(360/(2*vfFN)))),center=true,$fn=vfFN); 
...
module zteardrop(diameter,height)
{
	rotate(a=0, v=[0,0,1]) union()
	{
		...
		rotate(a=22.5, v=[0,0,1])cylinder(r=diameter/(2*cos(360/(2*vfFN))), h = height,center=true,$fn=vfFN);
	}
}
